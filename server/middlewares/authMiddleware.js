...
module.exports = async function authMiddleware(req, res, next) {
    try {
        ...
        if (!secret) {
            ...
            return res.status(500).json({ success: false, code: 'Server configuration error' });
        }
        ...
        if (!decoded || !decoded.id) {
            return res.status(401).json({ success: false, code: 'Invalid token' });
        }

        // confirm user still exists in DB
        // req.db is expected to exist (other controllers do the same)
        const [rows] = await req.db.query('SELECT id, name, surname, email, role, photo_url, status FROM users WHERE id = ?', [decoded.id]);
        if (!rows || rows.length === 0) {
            return res.status(401).json({ success: false, code: 'Invalid token (user not found)' });
        }
        ...
        if (err && (err.name === 'TokenExpiredError' || err.name === 'JsonWebTokenError')) {
            return res.status(401).json({ success: false, code: 'Invalid or expired token' });
        }
